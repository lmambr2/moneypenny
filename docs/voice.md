...
2. STT `http://stt-whisper:9000`, TTS `http://piper-tts:8880`, voice **`en_GB-cori-high`** (British high; medium is the fallback. Samples at [piper-samples](https://rhasspy.github.io/piper-samples/)).
...
- **Karaoke mode** (`!karaoke on` / Settings checkbox): ducks only to **80** so the track stays singable. `!karaoke off` restores the normal duck.
