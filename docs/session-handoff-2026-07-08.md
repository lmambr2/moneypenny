...
## 8. Follow-up (same day) — voice false positives

**Author:** Grok (xAI), driven by Lane Ambrose.

### 8.1 Audit correction

Re-parsed full `bot.log` Voice turns. The §5 claim "**0 commands executed**" for
the Toto Africa window was **wrong**:

| Transcript | Reply |
|---|---|
| `Any much? Money Penny, play Toto Africa.` | **Now playing: Toto - Africa** |
| `Money petty play tonneau Africa.` | Now playing: …Rat Rods for Africa (STT mangled title) |
| `Forget it. I think the problem is she's slow.` | `Usage: !forget …` (**false positive**) |

Opus path is healthy post-`974ea1d`. Residual pain is **command extraction**, not decode.

### 8.2 Other false positives seen in logs

- `Now I need to go to a room` → `!now` (and partial-safe)
- Mid-sentence `pod` → `pause` (alias scan)
- `Money peri, France` → historically `resume` (wake-bleed + peri alias; already partially gated)
- Bare `play` loops → LLM "Play what, precisely?"

### 8.3 Fix (uncommitted at write time — commit before deploy)

- `bot/src/voice/watchword.ts` — `voiceCommandShapeOk`, short-only transport scan,
  drop `now`/`queue` from partial-safe
- `bot/src/bot/voice/session.ts` — silence-tail allows `play <title>`, rejects bare play
- Tests green: **802** backend + **11** web

### 8.4 Deploy / verify

```bash
# from /home/lane/Projects/moneypenny
./scripts/deploy-to-pi.sh --files bot/src/voice/watchword.ts bot/src/bot/voice/session.ts --services bot
# then in TS: "Moneypenny play Toto Africa" / "Moneypenny pause" while music plays
# banter should NOT trigger now/forget/pause
```

---

*Generated by Grok agent session, 2026-07-08. Attribution: Grok (xAI), driven by Lane Ambrose.*
*§8 added later the same day after Pi log re-analysis.*