...
| Subsystem | Path | Owns |
|-----------|------|------|
| **TS6 client** | `bot/packages/ts6-client` (`@moneypenny/ts6-client`) | Dual-protocol connection, chat in/out, voice PCM in, file transfer, channel queries — import package root only; see [docs/ts6-client.md](./docs/ts6-client.md) |
...
- [2026-08-13] `control/tool-map.ts` `sourceFlags()` mapped only `local|youtube` — LLM `play_music` with `source=stream` dropped the `-s` flag and defaulted to local. → `sourceFlags` must handle all three platforms.
- [2026-08-14] `routeVoice` treated `ask`/`analyst`/`agent`/`intsum`/`aar` as deterministic known commands (manifest kind `router`, no CommandRegistry handler) → spoken/poked "ask …" replied `Unknown command. Try !help`. Chat `route()` special-cased them. Poke `!ask` re-prefixed to `!!ask`. → Shared `llmDecisionForCommand`; strip leading `!` in `routeVoice`.
- [2026-08-13] `LlmClient.chat` wrapping `fetchJson` failures as `new Error("LLM request failed: …")` stripped `HttpRequestError.status` — `FallbackLlmClient` never failed over on 429/502/503. → Rethrow the `HttpRequestError` so status-based retry works.
- [2026-08-13] `assertSafePlaybackTarget` treated every non-`http(s)` string as a MUSIC_DIR path — `file:///etc/passwd`, `rtmp://`, and `//127.0.0.1/…` passed the final ffmpeg gate. → Only real filesystem paths skip the SSRF check (`isLocalLibraryPath`).
- [2026-08-13] Qdrant renamed to `turbovec` but `url-guard` `BLOCKED_HOSTNAMES` still listed `qdrant` only — `isPublicPlaybackUrl("http://turbovec:6333")` was true. → Keep compose service names in the hostname blocklist.
