...
export class TS6HttpQuery {
  ...
  /** List clients including server-group memberships (TS6 needs the `-groups` flag). */
  async clientListWithGroups(sid = 1): Promise<HttpQueryResult> {
    return this.request("GET", `/1/clientlist?sid=${sid}&-groups`);
  }

  /** Single-client properties (includes `client_servergroups` and `cid`). */
  async clientInfo(clid: number, sid = 1): Promise<HttpQueryResult> {
    return this.request("GET", `/1/clientinfo?sid=${sid}&clid=${clid}`);
  }
