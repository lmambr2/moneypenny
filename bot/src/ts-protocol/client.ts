...
export class TS3Client extends EventEmitter {
  ...
  async connect(): Promise<void> {
    ...
    // pipeline can decode + end-point them. Only wired when the consumer
    // subscribes; otherwise these events are simply ignored.
    this.client.on("voiceData", (v: VoiceData) => {
      this.emit("voiceData", {
        clientId: v.clientId,
        codec: v.codec,
