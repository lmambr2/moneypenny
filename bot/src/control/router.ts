...
export interface LlmAssist {
  /** Plain-text Q&A for `!ask <question>` (no tools). userUid for Phase 7 memory. */
  ask(question: string, conversationId?: string, userUid?: string): Promise<string>;
...
export class ControlRouter {
  ...
  private async executeLlm(intent: LlmIntent, context: RouterContext): Promise<string | null> {
    ...
    if (intent.mode === "ask") {
      ...
      return this.llm.ask(intent.text, context.conversationId, context.invokerUid);
    }
