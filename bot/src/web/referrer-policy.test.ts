 ...
 * action). On 2026-05-27 the same-origin CSRF gate `app.use("/api",
 * csrfOriginCheck)` was added in server.ts, and the same day a
 * `<meta name="referrer" content="no-referrer">` was added to web/index.html
 * so cross-origin CDN cover thumbnails would load.
 *
 ...
 * origin", so POST /api/player/* (and every other POST/PUT/DELETE under
