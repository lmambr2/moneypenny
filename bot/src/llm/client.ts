...
export class LlmClient {
  ...
  constructor(options: LlmClientOptions = {}) {
    ...
    this.model = options.model || process.env.RKLLAMA_MODEL || "qwen3-1.7b";
  ...
  async chat(req: ChatCompletionRequest): Promise<ChatCompletionResponse> {
    ...
    try {
      ...
      this.logger?.warn({ err: err?.message, baseUrl: this.baseUrl }, "LLM chat request failed");
