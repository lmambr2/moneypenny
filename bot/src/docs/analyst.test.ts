...
describe("parseAnalystCommand", () => {
  it("parses task without flags", () => {
    ...
    expect(p).toEqual({ task: "summarise fleet doctrine", save: false, classification: "restricted" });
  });
  ...
  it("parses -s and class: flags", () => {
    ...
    expect(p).toMatchObject({ save: true, classification: "secret", task: "draft op brief for TF18" });
  });
