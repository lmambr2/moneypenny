...
//! Decode-then-classify DTX: packets ≤12 bytes can be valid encoded silence.
...
/// Returns `None` when the TOC/layout is invalid (JS returns null).
...
pub fn is_dtx_sized_packet(packet: &[u8]) -> bool {
...
mod tests {
    ...
    #[test]
    fn mode0_single_frame_returns_whole_packet() {
        let pkt = vec![0u8, 1, 2, 3];
        let frames = split_opus_frames(&pkt).unwrap();
    ...
    #[test]
    fn empty_is_none() {
        assert!(split_opus_frames(&[]).is_none());
    }
}
